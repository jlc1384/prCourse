...
use FirstBundle\Form\CourseType;
...
class DefaultController extends Controller
{
    ...
    public function nativeDQLWithModelAction() {
        $em = $this->getDoctrine()->getEntityManager();
        $coursesRepo = $em->getRepository("FirstBundle:Course");
        
        $courses = $coursesRepo->findCourseModelTest();

        echo "<hr>ALL COURSES SQL NATIVE USING MODEL:: </br>";
        foreach($courses as $cc) {
            echo "Id: ".$cc->getId()." -- Name: ".$cc->getName()." -- Desfripion: ".$cc->getDescription().
                " -- Start Date: ".$cc->getStartDate()->format('Y-m-d')." -- End Date: ".$cc->getEndDate()->format('Y-m-d')
                    ." -- Price: ".$cc->getPrice()." </br>";
    ...
    public function addCourseAction(Request $request) {
        ...
        return $this->render('FirstBundle:course:addCourse.html.twig', array(
